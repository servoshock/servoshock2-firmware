...
static BYTE epInterruptIn = 0x81;
static BYTE epInterruptOut = 0x01;
static BYTE epBulkIn = 0x82;
static BYTE epBulkOut = 0x02;
static CONNECTED_DEVICE ConnectedDevice = NONE;
...
BOOL USBHostGenericInitBT(BYTE address, DWORD flags, BYTE clientDriverID) {
    BYTE *pDesc;

    SetConnectedDeviceType(BLUETOOTH); //MH
    SetEpInterruptIn(0x81);
    SetEpInterruptOut(0x01);
    SetEpBulkIn(0x82);
    SetEpBulkOut(0x02);
...
#ifdef DEBUG_MODE
    UART2PrintString("HOST: USB BLUETOOTH INIT.\r\n"); //MH
...
BOOL USBHostGenericInitWire(BYTE address, DWORD flags, BYTE clientDriverID) {
    BYTE *pDesc;

    SetConnectedDeviceType(CONTROLLER); //MH
    SetEpInterruptIn(0x84);
    SetEpInterruptOut(0x03);
...
#ifdef DEBUG_MODE
    UART2PrintString("HOST: USB WIRE INIT.\r\n"); //MH
    UART2PrintString("GEN: USB Controller Client Initalized: flags=0x");
    UART2PutHex(flags);
    UART2PrintString(" address=");
    UART2PutDec(address);
    UART2PrintString(" VID=0x");
...
BOOL USBHostInitSpaceMouse(BYTE address, DWORD flags, BYTE clientDriverID) {
    BYTE *pDesc;
    UART2PrintString("Init 3D Mouse\n\r");
    
    SetConnectedDeviceType(SPACE_MOUSE); //MH
    SetEpInterruptIn(0x81);
    SetEpInterruptOut(0x02);
...
BOOL USBHostInitThrustmaster(BYTE address, DWORD flags, BYTE clientDriverID) {
    BYTE *pDesc;
    UART2PrintString("Init Thrustmaster\n\r");
    
    SetConnectedDeviceType(THRUSTMASTER); //MH
    SetEpInterruptIn(0x84);
    SetEpInterruptOut(0x03);
    ...
    switch (event) {
        ...
        case EVENT_TRANSFER:
            if ((data != NULL) && (size == sizeof (HOST_TRANSFER_DATA))) {
                ...
                if (((HOST_TRANSFER_DATA *) data)->bEndpointAddress == (USB_IN_EP | epInterruptIn))//YTS
                {
                ...
                } else if (((HOST_TRANSFER_DATA *) data)->bEndpointAddress == (USB_IN_EP | epBulkIn))//YTS
                {
                ...
                else if (((HOST_TRANSFER_DATA *) data)->bEndpointAddress == (USB_OUT_EP | epInterruptOut))//YTS
                {
                ...
                } else if (((HOST_TRANSFER_DATA *) data)->bEndpointAddress == (USB_OUT_EP | epBulkOut)) {
...
BYTE USBHostGenericRead(BYTE deviceAddress, void *buffer, DWORD length) {
    ...
    gc_DevData.rx1Length = 0;
    RetVal = USBHostRead(deviceAddress, USB_IN_EP | epInterruptIn, (BYTE *) buffer, length);
...
void USBHostGenericTasks(void) {
    ...
    if (gc_DevData.ID.deviceAddress && gc_DevData.flags.initialized) {
        ...
        if (gc_DevData.flags.rxBusy) {
            if (USBHostTransferIsComplete(gc_DevData.ID.deviceAddress, USB_IN_EP | epInterruptIn, &errorCode, &byteCount)) {
        ...
        if (gc_DevData.flags.txBusy) {
            if (USBHostTransferIsComplete(gc_DevData.ID.deviceAddress, USB_OUT_EP | epInterruptOut, &errorCode, &byteCount)) {
...
BYTE USBHostGenericWrite(BYTE deviceAddress, void *buffer, DWORD length) {
    ...
    gc_DevData.flags.txBusy = 1;
            RetVal = USBHostWrite(deviceAddress, USB_OUT_EP | epInterruptOut, (BYTE *) buffer, length); //ep = 3
    if (RetVal != USB_SUCCESS) {
...
BYTE USBHostGenericAclWrite(BYTE deviceAddress, void *buffer, DWORD length) {
    ...
    gc_DevData.flags.tx2Busy = 1;
            RetVal = USBHostWrite(deviceAddress, USB_OUT_EP | epBulkOut, (BYTE *) buffer, length);
    if (RetVal != USB_SUCCESS) {
...
BYTE USBHostGenericAclRead(BYTE deviceAddress, void *buffer, DWORD length) {
    ...
    gc_DevData.flags.rx2Busy = 1;
            gc_DevData.rx2Length = 0;
            RetVal = USBHostRead(deviceAddress, USB_IN_EP | epBulkIn, (BYTE *) buffer, length);
    if (RetVal != USB_SUCCESS) {
...
/*****************Added Functions by MH***********************/
BYTE GetEpInterruptIn(void) {

    return epInterruptIn;
}

BYTE GetEpInterruptOut(void) {

    return epInterruptOut;
}

BYTE GetEpBulkIn(void) {

    return epBulkIn;
}

BYTE GetEpBulkOut(void) {

    return epBulkOut;
}

void SetEpInterruptIn(BYTE ep) {

    epInterruptIn = ep;
}

void SetEpInterruptOut(BYTE ep) {

    epInterruptOut = ep;
}

void SetEpBulkIn(BYTE ep) {

    epBulkIn = ep;
}

void SetEpBulkOut(BYTE ep) {
...
CONNECTED_DEVICE GetConnectedDeviceType(void) {

    return ConnectedDevice;
};

void SetConnectedDeviceType(CONNECTED_DEVICE type) {
