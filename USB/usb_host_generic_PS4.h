...
#define FAIL_COUNTER 5 //number of times a read/write can be busy before the busy flag is reset for a retry.

//  These endpoints are hard-coded for now, may cause problems if expected endpoints
//on the controller are not consistent among DualShock3. -MH
...
#define EVENT_GENERIC_TX2_DONE (EVENT_GENERIC_BASE+EVENT_GENERIC_OFFSET+5)//YTS
...
typedef enum _CONNECTED_DEVICE //MH, keeps track of what device is connected to the USB port
{
    NONE,
    BLUETOOTH,
    CONTROLLER,
