...
PAT                 27-Jan-2010  Added UART2GetBaudError() for dynamic checking
 ...
 MH                 22-Oct-2012 Changed BAUDRATEREG2 formula
 *                              Fixed bug in UART2PutDec
 *                              Added UART2PutSDec
 *                              Added UART2PutDecInt(unsigned int dec);
 *                              Added UART2PutDecSInt(signed int dec);
 ********************************************************************************
...
#else
#define BAUDRATEREG2        (((GetInstructionClock())/(BRG_DIV2 * BAUDRATE2)) - 1) //MH, use new formula
#endif
 ...
 *******************************************************************************/
char UART2IsPressed() {
    if (IFS1bits.U2RXIF == 1 || U2STAbits.URXDA)
        return 1;
    return 0;
}
...
void UART2PutDec(unsigned char dec) {
    ...
    else
    {
        ...
        while (digit)
        {
            UART2PutChar(res / digit + '0');
            res = res - (res / digit)*digit;
            digit = digit/10;
        }
    }
}

void UART2PutSDec(signed char dec) {
    unsigned char res;
    if (dec < 0) {
        res = ~dec + 1;
        UART2PutChar('-');
...
void UART2PutDecInt(unsigned int dec) {
    unsigned int res;
...
void UART2PutDecSInt(signed int dec) {
    unsigned int res;
    if (dec < 0) {
        res = ~dec + 1;
        UART2PutChar('-');
...
void UART2PutDecLong(unsigned long dec) {
    unsigned long res;
...
void UART2PutDecSLong(signed long dec) {
    unsigned long res;
    if (dec < 0) {
        res = ~dec + 1;
        UART2PutChar('-');
